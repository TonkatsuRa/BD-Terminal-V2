// js/features/sites.js — remote BRE site connections (/CONNECT, /DISCONNECT,
// /SITE STATUS, /WELCOME), the connected-site record store, and the animated
// network/data-access logs shared with the database mount sequence.

import { motion } from '../core/effects.js';
import { AudioEngine } from '../core/audio.js';
import { AppState, setAppState, onUiSync } from '../core/state.js';
...
import { printNetworkUnavailable } from '../terminal/messages.js';
import { runTerminalProgressBar, bootStyleStep, bootStyleHeader } from '../terminal/progress.js';
import { setConnectedSiteStatusProfile, connectedSiteStatusProfile, CONNECTED_SITE_SESSION_KEY } from './status.js';
// Runtime-only circular import (database.js ↔ sites.js); calls happen after
// both modules have evaluated.
import { rebuildDatabaseIndex, entryTopic, isDatabaseLoaded } from './database.js';
...
const DEFAULT_BRE_SITES = [
    ...
    id, displayName, code, database, statusProfile, floorplan
}));

// Per-site operator hint printed at the bottom of /WELCOME. Seeds the puzzle
// chain — BRE-06 receives the player on first connect.
const SITE_OPERATOR_HINTS = {
    'BRE-06': [
        ['', ''],
...
export function getConnectedSiteDatabase() {
    return connectedSiteDatabase;
}

export function getConnectedSiteDatabaseEntries() {
...
/* ------------------------------------------------------------------ *
 * Fallback content when site files cannot be fetched
 * ------------------------------------------------------------------ */
...
export async function connectBreSite(ctx = {}) {
...
export function disconnectConnectedSite(options = {}) {
    if (!connectedSite) {
...
export function showConnectedSiteStatus() {
...
export function showConnectedSiteWelcome() {
    clearOutput({ force: true });
    print('');
...
/* ------------------------------------------------------------------ *
 * Animated service logs
 * ------------------------------------------------------------------ */

export async function runNetworkServicesLog() {
    await bootStyleHeader('', '');
...
export async function runRemoteDataAccessLog(operation = 'DATA') {
...
export async function runDatabaseLoadLog(source = 'DATABASE', entries = 0, slotNumber = 0) {
...
/* ------------------------------------------------------------------ *
 * UI sync (site indicators + floorplan preview)
 * ------------------------------------------------------------------ */

export function updateConnectedSiteUi() {
...
// Keep the indicators in sync with every state change.
onUiSync(() => updateConnectedSiteUi());

/* ------------------------------------------------------------------ *
 * Session snapshots (used by session-restore and tool screens)
 * ------------------------------------------------------------------ */

export function snapshotConnectedSite() {
    return {
        ...
        statusProfile: connectedSiteStatusProfile && connectedSiteStatusProfile.loaded
            ? connectedSiteStatusProfile
            : null
    };
}

export function restoreConnectedSiteFromSnapshot(snapshot, options = {}) {
...
export function restoreConnectedSiteForToolScreen() {
    try {
...
export function getConnectedSiteDebugSnapshot() {
    return {
