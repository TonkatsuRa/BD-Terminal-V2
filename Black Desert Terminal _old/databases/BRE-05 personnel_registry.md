---
id: database1
title: Personnel Registry
description: Employee, contractor, and missing staff notes for Black Desert Research.
password: KESS-C12
---

<!--
DATABASE EDITING NOTES
Use this structure for each entry:
Topic: Short searchable topic
ID or Person: Optional ID, employee name, or subject
Date: YYYY-MM-DD or blank
Access: Employee, Elevated, or Admin
Keywords: keyword one; keyword two; keyword three
Message:
Full readable message text begins here.

SEARCH checks Topic, Date, and Keywords.
FSEARCH also checks ID or Person and Message, if your clearance allows it.
Comments are stripped before terminal display.
-->
## Category: PERSONNEL

### Entry: Director Halden Voss
Topic: Director Halden Voss
ID or Person: personnel-voss
Date: 
Access: Employee
Keywords: director; command; ares; executive
Message:
Facility Director assigned by Ares Macrotechnology Special Projects.
Last confirmed in Command Core during external relay failure.
Personnel compliance reports describe Voss as efficient, intolerant of delay, and unusually calm after Containment Cell alarms.
Related: payroll-audit, executive-evacuation

### Entry: Dr. Mara Kess
Topic: Dr. Mara Kess
ID or Person: personnel-kess
Date: 
Access: Employee
Keywords: xenobiology; lab arc; missing; biomonitor
Message:
Senior researcher assigned to Lab Arc C-12.
Reported seal drift three times before the diagnostic bus marked the issue as non-critical.
Current location unknown. Life-sign system shows intermittent match in service tunnels.
Related: specimen-c12, medbay-quarantine
Redacted note: Last twenty minutes of biomonitor telemetry removed by administrator action.

### Entry: Security Chief Anika Vale
Topic: Security Chief Anika Vale
ID or Person: personnel-vale
Date: 
Access: Employee
Keywords: security; defense grid; armed; patrol
Message:
Responsible for autonomous defense oversight and physical security rotation.
Last filed an order to keep defense systems armed until off-site confirmation returns.
No off-site confirmation has been received.
Related: perimeter-drone, vault-servo-3

## Category: CONTRACTORS

### Entry: Rourke Industrial Crew
Topic: Rourke Industrial Crew
ID or Person: contractor-rourke
Date: 
Access: Employee
Keywords: generator; coolant; filters; maintenance
Message:
Third-party maintenance crew hired for generator coolant filter replacement.
Work order remains open. Crew badges last pinged near Generator Plant service access.
One badge continues to move without a matching heartbeat signal.
Related: coolant-filter, turbine-rpm

### Entry: Jackpoint Courier Null-Slate
Topic: Jackpoint Courier Null-Slate
ID or Person: courier-null-slate
Date: 
Access: Elevated
Keywords: courier; sealed case; external relay
Message:
Courier listed under temporary Ares shell authorization.
Manifest indicates delivery of a sealed data case to Black Desert Research.
Security log shows the courier arrived nine minutes before external relay failure.
Related: relay-failure, sealed-case

## Category: CONFIDENTIAL

### Entry: Executive Evacuation Hold
Topic: Executive Evacuation Hold
ID or Person: executive-evacuation
Date: 
Access: Admin
Keywords: executive; evacuation; shareholder; directive
Message:
Executive evacuation was authorized for command staff only.
Remaining personnel were instructed to preserve facility assets and await retrieval.
Retrieval window expired. Directive remains active.
Related: personnel-voss
Redacted note: Names of evacuated personnel suppressed under Ares shareholder-risk protocol.

### Entry: Cross-Site Credential Mirror
Topic: Cross-Site Credential Mirror
ID or Person: cross-site-credential-mirror
Date: 
Access: Elevated
Keywords: credential; mirror; bre-04; bre-03; vale; handoff
Message:
Security Chief Vale maintained mirrored credentials for the two sites she rotated through.
Mirror table (last successful sync):

  - GLASSHOUSE / CONTAINMENT  ->  DELTA-5891   (BRE-04 intranet)
  - RIDGE / OBSERVATION POST  ->  CHARLIE-1206 (BRE-03 intranet)

Note appended in Vale's handwriting:
"Brandt's clearance was issued through the ridge-line observation post.
 If you ever need to read his log, you go through the ridge first."
Related: personnel-vale, spirit-log-glass-observation-2081-03-09
