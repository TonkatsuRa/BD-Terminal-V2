...
// Hex parts decode to "AresAres123". Keep in sync with the password documented
// in Zip PW Databases.txt and TERMINAL_COMMANDS_PASSWORDS_GUIDE.txt, and with
// ENCRYPTION_KEY = 'Ares' in js/utils.js.
...
// A database is visible at the LOAD menu when it belongs to the currently
// connected site (or is unconditionally available). Three layered sources of
// "which sites does this database belong to" are consulted, in order:
//   1. The manifest entry's `sites` array (highest priority).
//   2. A cached `sites:` line from the .md file's frontmatter.
//   3. The filename prefix: "Terminal " → always; "BRE-XX " → that site.
...
// Cache of {filename → sites[]|null} parsed from .md frontmatter. Filled on
// demand by prefetchSitesMetadata; `null` means "no sites: line in this file".
const sitesCache = new Map();

/**
 * Parse a `sites:` line from the YAML-ish frontmatter of a database .md file.
...
/**
 * Compute the set of sites at which a manifest entry should be visible.
 * Priority: manifest.sites > cached frontmatter sites > filename prefix > default.
 *
 * @param {Object} entry - a manifest entry (`{file, sites?, ...}`)
 * @returns {string[]} list of site ids (may contain ALWAYS_SITE or "*")
 */
...
/**
 * Filter a manifest down to the entries visible at the given connected site.
 * Strict gating: an unknown / empty site id collapses to ALWAYS_SITE entries
 * only. The sentinel "*" in an entry's sites array means always-visible.
 *
...
/**
 * Fetch and cache the `sites:` field from every .md file that doesn't already
 * have a manifest-level `sites` array or a cached entry. Each file is fetched
 * at most once per session. Per-file errors are silently absorbed.
 *
 * @param {Array<Object>} manifest
 */
async function prefetchSitesMetadata(manifest) {
    ...
    const needsFetch = list.filter(entry => {
        ...
        if (sitesCache.has(file)) return false;
        return true;
    });
...
async function showDatabaseSelector() {
    ...
    // Read any in-file `sites:` declarations for entries that need them. In
    // the common case (manifest field present, or recognisable filename
    // prefix), this is a no-op; orphan entries get a single fetch the first
    // time the menu is opened, then are cached for the session.
