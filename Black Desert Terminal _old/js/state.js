...
/**
 * Type plain text into `element` one character at a time using requestAnimationFrame.
 * Honours the user's prefers-reduced-motion preference (instant render if set).
 *
 * @param {HTMLElement} element - target; existing children are cleared
 * @param {string} text - the text to reveal
 * @param {Object} [options]
 * @param {string} [options.activeClass='terminal-typewriter-active'] - CSS class
 *   added to the element while typing is in progress
 * @param {number} [options.interval] - ms between characters (fallback if no charsPerSecond)
 * @param {number} [options.charsPerSecond] - typing speed; takes precedence over `interval`
 * @param {number} [options.maxCharsPerFrame=1] - hard cap on chars revealed per RAF tick
 * @param {() => boolean} [options.shouldCancel] - polled each frame; if it returns
 *   true the animation cancels and the promise resolves with `cancelled: true`
 * @param {(frame: number|null) => void} [options.onFrame] - called with the current
 *   RAF id (or null on finish/cancel) so callers can store it for external cancellation
 * @param {(index: number, char: string) => void} [options.onChar] - called once per
 *   revealed character; used by the terminal to drive AudioEngine.keyClick cadence
...
/**
 * Variant of {@link typeTextSmooth} for lines that contain inline-color spans.
 * Builds the DOM up-front (text nodes for plain segments, <span class="t-X">
 * wrappers for colored ones) and reveals characters across them so the typewriter
 * cadence is preserved even as the visible color changes mid-line.
 *
 * @param {HTMLElement} element - target; existing children are cleared
 * @param {Array<{text: string, className: string|null}>} segments - pre-parsed
 *   output from `parseColorSegments` (terminal.js)
 * @param {Object} [options] - same option shape as {@link typeTextSmooth}
 * @returns {Promise<{completed: boolean, cancelled: boolean}>}
 */
...
// FALLBACK_DATABASE_MANIFEST mirrors databases/manifest.json. Used when the
// manifest fetch is blocked (e.g. opening index.html via file://). Filename
// prefixes encode the site-gating: "Terminal " → always-visible, "BRE-XX " →
// only visible when connected to that site. The Metaplanar Archive carries an
// explicit `sites` array as a multi-site override example.
const FALLBACK_DATABASE_MANIFEST = [
    ...
    },
    {
        ...
        description: 'Specimen vaults, prototype lockers, and archived lab inventory at Meridian Extraction.',
        file: 'BRE-02 research_assets.md'
    },
    {
        id: 'maintenance_queue',
        displayName: 'Maintenance Queue',
        description: 'Power, life-support, access, and generator maintenance backlog at Meridian Extraction.',
        file: 'BRE-02 maintenance_queue.md'
    },
    {
        id: 'ares_03_shadow_spirits_inhabitants',
        displayName: 'Shadow Spirits and Inhabitants',
        description: 'Field notes and warning reports on entities and inhabitants observed from Khepri.',
        file: 'BRE-03 shadow_spirits_inhabitants_de.md'
    },
    {
        id: 'ares_database1_metaplanar_access_de',
        displayName: 'Metaplanar Observations (DE)',
        description: 'German-language observation logs of metaplanar phenomena tracked from Khepri.',
        file: 'BRE-03 metaplanar_observations_de.md'
    },
    {
        id: 'security_incidents',
        displayName: 'Security Incidents',
        description: 'Patrol reports, alarm events, and defense-grid irregularities at Glasshouse.',
        file: 'BRE-04 security_incidents.md'
    },
    {
        id: 'confidential_archive',
        displayName: 'Confidential Archive',
        description: 'Restricted executive notes, redacted incidents, and sealed directives at Glasshouse.',
        file: 'BRE-04 confidential_archive.md'
    },
    {
        id: 'ares_05_security_logbook',
        displayName: 'ARES Security Logbook',
        description: 'Security events, patrol reports, Firewatch protocols, and automated alarms at Glasshouse.',
        file: 'BRE-04 security_logbook_de.md'
    },
    {
        id: 'outpost_relay_logs',
        displayName: 'Outpost Relay Logs',
        description: 'Remote station, drone uplink, and mesh-network records routed through Boreline.',
        file: 'BRE-05 outpost_relay_logs.md'
    },
    {
        id: 'personnel_registry',
        displayName: 'Personnel Registry',
        description: 'Employee, contractor, and missing staff notes maintained at Boreline Relay.',
        file: 'BRE-05 personnel_registry.md'
    },
    {
        id: 'ares_06_research_laboratory',
        displayName: 'ARES Research Laboratory',
        description: 'Laboratory logs and internal analysis of portals, black sand, and anomalies at Orpheus.',
        file: 'BRE-06 research_laboratory_de.md'
    },
    {
        id: 'ares_07_psychiatric_ai_reports',
        displayName: 'Psychiatric AI Reports',
        description: 'Automated psychological care, stress, and risk reports from CARE-9 at Orpheus.',
        file: 'BRE-06 psychiatric_ai_reports_de.md'
    }
];
// Menu state
